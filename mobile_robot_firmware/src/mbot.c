...
float r_motor_v_to_d(float vel){
    float a = 0.911;   // -0.8802
    float b = 0.1585;     // 0.115
    float d = a*(vel)+b;
    return d;
}
...
bool timer_cb(repeating_timer_t *rt)
{
    ...
    if (comms_get_topic_data(MBOT_TIMESYNC, &received_time))
    {
        ...
        float s_L, s_R, delta_x, delta_y;

        s_R = enc2meters*current_encoders.right_delta;
        s_L = enc2meters*current_encoders.left_delta;
        delta_theta = (s_R-s_L)/WHEEL_BASE;
        //WHEEL_BASE = 0.133
        //WHEEL_RADIUS = 0.0415
        delta_d = (s_R+s_L)/2;
        delta_x = delta_d * cos(current_odom.theta+delta_theta/2);
        delta_y = delta_d * sin(current_odom.theta+delta_theta/2);

        current_odom.x += delta_x;
        current_odom.y += delta_y;
        current_odom.theta += delta_theta;
        current_odom.theta = clamp_theta(current_odom.theta);
        ...
        {
            printf("trans vel: %f", current_cmd.trans_v);
            printf(" ang vel: %f \n", current_cmd.angular_v);
            ...
            left_sp = current_cmd.trans_v + (wheel_base/2.)*current_cmd.angular_v;
            right_sp = current_cmd.trans_v - (wheel_base/2.)*current_cmd.angular_v;
            printf("l vel: %f", left_sp);
            printf(" r vel: %f \n", right_sp);
            ...
            else
            {
                ...
                // float vel = 0.3;
                // l_duty = l_motor_v_to_d(vel);
                // r_duty = r_motor_v_to_d(vel);
                // l_encoder_reading;
                // float l_wheel_speed;
                // r_encoder_reading;
                // float r_wheel_speed;
                ...
                rc_motor_set(1, (int32_t)(l_duty*INT_16_MAX)); // left
                rc_motor_set(3, -(int32_t)(r_duty*INT_16_MAX)); // right
                if(flag){
                    l_duty = l_motor_v_to_d(left_sp);
                    r_duty = r_motor_v_to_d(right_sp);
                    flag = false;
                }
                // l_encoder_reading = -rc_encoder_read_delta(1);
                // r_encoder_reading = -rc_encoder_read_delta(3);
                //printf("!!!!!!!!!!!!!!\n");
                printf("%f\t%f\n", enc_delta_l, enc_delta_r);
                l_wheel_speed = RPM_conversion_factor * enc_delta_l * (2*M_PI*r/60.) * MAIN_LOOP_HZ; // convert to m/s
                r_wheel_speed = RPM_conversion_factor * enc_delta_r * (2*M_PI*r/60.) * MAIN_LOOP_HZ; // convert to m/s
                // printf("%f\t%f\n", l_encoder_reading, r_encoder_reading);
                ...
                rc_filter_pid(&right_pid,
                                ...
                                1.0 / 5,
                                1.0 / MAIN_LOOP_HZ);
                l_duty += rc_filter_march(&left_pid, left_sp - l_wheel_speed);
                r_duty += rc_filter_march(&right_pid, right_sp - r_wheel_speed);
                //  r_duty += rc_filter_march(&right_pid, l_wheel_speed - r_wheel_speed);
                l_duty = clamp_duty(l_duty);
                r_duty = clamp_duty(r_duty);
                rc_motor_set(1, (int32_t)(l_duty*INT_16_MAX));
                rc_motor_set(3, -(int32_t)(r_duty*INT_16_MAX));

                printf("%f\t%f\t%f\t%f\n", l_duty, r_duty, l_wheel_speed, r_wheel_speed);
...
int main()
{
    ...
    while (running)
    {
        // printf("\033[2A\r|      SENSORS      |           ODOMETRY          |     SETPOINTS     |\n\r|  L_ENC  |  R_ENC  |    X    |    Y    |    θ    |   FWD   |   ANG   \n\r |%7lld  |%7lld  |%7.3f  |%7.3f  |%7.3f  |%7.3f  |%7.3f  |" , \
        // current_encoders.leftticks, current_encoders.rightticks, current_odom.x, current_odom.y, current_odom.theta, current_cmd.trans_v, current_cmd.angular_v);
    }
}
